<?php
    ...
    if($_POST['addcollection']){
     ...
     /// REMOVE PHOTO ///
     if($_POST['removecollection']){

        $referenceid = $_POST['referenceid'];
        $referencefolder = $_POST['referencefolder'];

        $photoremove = $_POST['photoremove'];

        for($i=0;$i<count($photoremove);$i++){

        $del_id = $photoremove[$i];
        $sqldelete = "DELETE FROM test WHERE img = '$del_id'";
...
<body id="page-top"  style="background-color: rgb(218,207,207);padding: 0px;">
    <div id="wrapper">
        <nav class="navbar navbar-dark navbar-expand float-right align-items-start sidebar sidebar-dark accordion bg-gradient-primary p-0" style="background-image: url(&quot;none&quot;);background-color: rgb(239,210,106);">
            <div class="container-fluid d-flex flex-column p-0">
                ...
                <br><div class="text-center d-none d-md-inline"><button class="btn rounded-circle border-0" id="sidebarToggle" type="button" style="background-color: rgb(78,115,223);"></button></div>
            </div>
        </nav>
        <div class="d-flex flex-column" id="content-wrapper">
                ...
                <nav class="navbar navbar-light navbar-expand bg-white shadow mb-4 topbar static-top" style="background-image: url(&quot;assets/img/orange.png&quot;);">
                    <div class="container-fluid"><button class="btn btn-link d-md-none rounded-circle mr-3" id="sidebarToggleTop" type="button"><i class="fas fa-bars"></i></button><img style="width: 73px;height: 67px;margin: 5px;" src="assets/img/coe%20logo.png">
                        ...
                        <ul class="nav navbar-nav flex-nowrap ml-auto">
                            ...
                            <div class="nav-item dropdown no-arrow"><a class="dropdown-toggle nav-link" data-toggle="dropdown" aria-expanded="false" href="#"><span class="d-none d-lg-inline mr-2 text-gray-600 small" style="font-family: Nunito, sans-serif; font-size:12px;"><?php echo $last_name; ?>, <?php echo  $first_name; ?></span><img class="border rounded-circle img-profile" src="<?php echo "../../profileimages/" . $profile ?>" style="margin-top:1px;"></a>
            ...
            <div class="profile-card" style="width: 901px;margin-left: 240px;margin-top: 45px;">
            ...
            <div class="container-fluid">
                ...
                <?php if(substr($user_name,0,5)=='ADMIN'){
                  echo" <br><button class='btn btn-outline-primary text-truncate float-none float-sm-none add-another-btn' data-bs-hover-animate='pulse'
                   data-toggle='modal' data-target='#editphoto' type='button' style='margin: 4px;'>Upload Photos<i class='fas fa-pen edit-icon'></i></button>";
                }?>

                 <?php if(substr($user_name,0,5)=='ADMIN'){
                  echo" <button class='btn btn-outline-primary text-truncate float-none float-sm-none add-another-btn' data-bs-hover-animate='pulse'
                   data-toggle='modal' data-target='#removephoto' type='button' style='margin: 4px;'>Remove Photos   <i class='fas fa-trash'></i></button>";
                }?>
             ...
             <!----------- -----------    DISPLAY PICTURES------------------------------->
                ...
                <div class='gallery' style="center">
                    ...
                    $selectresults = $conn->query($selectimage);
                    if ($selectresults->num_rows > 0) {
                    // output data of each row
                        while($imgrow = $selectresults->fetch_assoc()) {
                        $img = $imgrow['img'];
                        $colid = $imgrow['collection_id'];
                        echo " <div class='img-hover-zoom--colorize'> <img src='../../collectionimage/coverimage/$folder/$img'> </div>";
                        }
                    }
                ...
                <div class="row">
                    <div class="col">
                        ...
                        <div class="modal fade centro" role="dialog"
                            tabindex="-1" id="removephoto">

                            <div class="modal-dialog" role="document">

                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h4>Remove Photos</h4><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                                    </div>
                                    <div class="modal-body">
                                        <input type="hidden" name="referenceid" value="<?php echo $id; ?>">
                                        <input type="hidden" name="referencefolder" value="<?php echo $folder; ?>">
                                        ...
                                        <table class="table table-bordered table-hover">
                                            <thead class="bill-header cs">
                                                <tr>
                                                    <th style="width: 888px;font-size: 18px;color: rgb(239,210,106);"> Uploaded Photos</th>
                                                    <th id="trs-hd" class="col-lg-1" style="color: rgb(239,210,106);font-size: 18PX;height: 38px;width: 200px;">Remove</th>
                                                </tr>
                                            </thead>
                                            <tbody>

                                                <?php
                                                    $selectphotos = "SELECT img FROM test where collection_id='$id'";
                                                    $photosresult = $conn->query($selectphotos);
                                                ...
                                                <tr>
                                                    <td> <img class="img-thumbnail" style="width: 200px;height:200px; margin-left: auto; margin-right: auto; display:block;" src="<?php echo "../../collectionimage/coverimage/$folder/" . $photos ?>"> </td>
                                                    <td> <br><br><br><input type="checkbox" name="photoremove[]" style="width: 25px; height: 25px;" value="<?php echo $photos; ?>"> </td>
                                                </tr>
                                                <?php }} ?>
                                            </tbody>
                                    ...
                                    <div class="modal-footer"><input class="btn btn-danger" type="submit" value="Remove" name="removecollection"></div>
                                    </div>
                                </div>
                            </div>
                        </form>
                            ...
                            <div class="modal-dialog" role="document">
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <label id="new_imagelabel" class="btn btn-info "style="display:block;">Select all the Photos that will be uploaded</label>
